...
<template>
  <section aria-label="演化历史">
    <p class="text-xs text-ink-dim">
      已完成 {{ state.history.length }} / {{ total }} 代，最新的一代排在最前面。
    </p>

    <ol v-if="rows.length" class="mt-3 space-y-2.5" aria-label="物种演化时间线">
      <li v-for="row in rows" :key="row.generation" class="flex gap-3">
        <div class="flex w-14 shrink-0 flex-col items-center pt-1">
          <span class="text-[11px] font-semibold tabular-nums text-ink-muted"
            >第 {{ row.generation }} 代</span
          >
          <span aria-hidden="true" class="mt-1 w-px flex-1 bg-frame-line" />
        </div>
        <div class="min-w-0 flex-1 rounded-xl border border-frame-line bg-black/20 p-2.5">
          <p class="flex flex-wrap items-center gap-x-2 text-sm font-medium text-ink">
            <span aria-hidden="true">{{ row.environmentIcon }}</span>
            <span>{{ row.environmentName || environmentNames[row.environmentId] || '环境' }}</span>
            <span v-if="row.eventName" class="text-ink-muted">· {{ row.eventName }}</span>
          </p>
          <p v-if="row.result" class="mt-1 text-xs tabular-nums text-ink-muted">
          ...
          <p v-if="row.expressed.length" class="mt-1 text-[11px] text-ink-muted">
            表达：{{ row.expressed.join(' · ') }}
          </p>
          <ul v-if="row.entries.length" class="mt-1.5 space-y-1">
            <li
              v-for="(entry, index) in row.entries"
              :key="`${entry.kind}-${index}`"
              class="text-[11px] leading-relaxed text-ink-muted"
            >
              <span aria-hidden="true">{{ kindIcons[entry.kind] }}</span>
              <span class="font-medium text-ink">{{ entry.title }}</span>
              <span class="text-ink-dim"> — {{ entry.detail }}</span>
            </li>
          </ul>
        </div>
      </li>
    </ol>
    <p v-else class="mt-3 text-xs text-ink-dim">完成第一个世代后，这里会开始记录物种的演化过程。</p>

    <details class="mt-3 border-t border-frame-line pt-2">
      <summary class="cursor-pointer text-xs text-ink-dim">
        全部世代数据表 · {{ state.history.length }} 代
      </summary>
      <div class="mt-2 overflow-x-auto">
        <table class="w-full min-w-150 text-left text-[11px]">
          ...
          <tbody>
            <tr
              ...
              class="border-t border-frame-line"
            >
              ...
              <td class="table-cell text-right tabular-nums text-emerald-300">
                +{{ result.births }}
              </td>
              <td class="table-cell text-right tabular-nums text-orange-200">
                −{{ result.totalDeaths }}
              </td>
              <td class="table-cell text-right font-medium tabular-nums text-ink">
                {{ result.populationAfter }}
              </td>
            </tr>
          </tbody>
        </table>
        <p v-if="!state.history.length" class="py-2 text-[11px] text-ink-dim">
          完成第一个世代后，这里会记录物种的变化。
        </p>
      </div>
    </details>
